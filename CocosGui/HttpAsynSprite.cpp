...
int progressFunc(void *ptr, double totalToDownload, double nowDownloaded, double totalToUpLoad, double nowUpLoaded)
{
    ...
    /*
    if ((static_cast<AssetsManager*>(ptr))->_cancelDownload)
    {
        curl_easy_pause((static_cast<AssetsManager*>(ptr))->_curl, CURLPAUSE_RECV);
        curl_easy_cleanup((static_cast<AssetsManager*>(ptr))->_curl);
         fclose((static_cast<AssetsManager*>(ptr))->_fp);
        return 0;
    }
    */
